...
export const Calendar = () => {
  const { calendarData, getCalendarData, getDaysInMonth, getMonthName } =
    useCalendar();

  const featuresInfoTable = [
    { label: "Nome entrega" },
    { label: "Etapa" },
    { label: "Etapa Status" },
  ];
        ...
        <Fragment key={idx_calendarData.toString()}>
          {idx_calendarData <= 0 && (
            <div className="flex flex-col min-w-[300px] w-[300px] border-r border-default">
              <div className="flex items-center justify-center h-[97px] border-b-default border-b">Projeto 1</div>
          ...
          <div className="w-max flex flex-col">
            <div className="w-full flex items-center justify-center h-14 border-r-default border-r">{calendar.month}</div>
            <div className="w-full flex border-b-default border-r-default border-b border-r text-xs">
              {calendar.days.map((day, idx) => (
                <div key={day + idx} className="w-14 min-w-[56px] flex items-center justify-center text-xs border-r-default border-r last:border-r-0">
                  <div className="w-10 h-10 flex items-center justify-center">{format(day, "d")}</div>
                </div>
              ))}
            </div>
              ...
              return (
                <div className="flex" key={feature.label + idx_feature}>
                  <div
                    ...
                    style={{ height: idx_feature === opened ? 106 : 56 }}
                  >
                    {calendar.days.map((day, idx_feature) => (
                      <div key={day + idx_feature} className={`w-14 min-w-[56px] flex items-center justify-center text-xs border-r-default border-r last:border-r-0`} style={{ borderRight: "none" }}>
                        {isDayInEvent(
                          day,
                          feature.event.start,
                          feature.event.end
                            ...
                            <div
                              className={`w-14 h-[70%] z-40 bg-primary flex justify-center ${isEqual(new Date(day), feature.event.start) && "rounded-l-xl ml-[6px]"} ${isEqual(new Date(day), feature.event.end) && "rounded-r-xl mr-[6px]"}`}
                            />
                          )}
                        {isToday(day) && (
